...
Integrando la ecuaci\'{o}n \ref{triangular} se obtiene la funci\'{o}n de 
...
\label{integralsintrans}
	\mathcal{E} \{ T \} = \int\limits_{\mathcal{D}} \min\{ \max \{ x_4, x_5 \} , x_3 , \max \{x_2, x_1\}\} \\ f_{x_1}(x_1) f_{x_2}(x_2) f_{x_3}(x_3) f_{x_4}(x_4) f_{x_5}(x_5) d^5x
\end{equation}
...
Se obtiene el determinante jacobiano de la transformaci\'on $J$:
\begin{equation}
	\label{jacobiano}
	J =
		\begin{vmatrix}
...
\label{integralcontrans}
	\mathcal{E} \{ T \} =	\int\limits_{[0,1]^6} \min\{ \max \{ 30 u_4 + 20, 5 u_5 + 7 \} , 12 u_3 + 72 , \max \{- T_m \ln (1 - u_2), - T_m \ln (1 - u_1)\}\} d^5u
\end{equation}

Para la realizaci\'on de la simulaci\'on del sistema mediante el m\'etodo de Montecarlo 
se utiliza el el generador de n\'umeros pseudo-aleatorios de L'Ecuyer obtenido en la secci\'on \ref{pruebagenerador}
...
$S = 0.86899$ horas. Resultando que para un nivel de significaci\'on del $5\%$, el tiempo medio de vuelo
...
aleatorios de L'Ecuyer, se desprende que es un buen generador. Ya que no solo no se observan hiperplanos en los gr\'{a}ficos sino que tambi\'{e}n aprob\'{o} los test de 
$\chi^{2}$ y Kolmogorov-Smironov.\\ 
